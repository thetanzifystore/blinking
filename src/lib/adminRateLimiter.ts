// @ts-nocheck
// Use require for the redis helper so tests using ts-node + mocha can require this file
const { getRedisClient } = require('./redis');
// lightweight metrics/reporting hooks (pluggable)
...
function getConfig() {
  ...
  const DEFAULT_WINDOW_MS = Number(process.env.ADMIN_RATE_LIMIT_WINDOW_MS) || 60 * 1000;
  const BACKOFF_BASE_MS = Number(process.env.ADMIN_RATE_LIMIT_BACKOFF_BASE_MS) || 1000; // base backoff 1s
  const BACKOFF_MULTIPLIER = Number(process.env.ADMIN_RATE_LIMIT_BACKOFF_MULTIPLIER) || 2;
  const BACKOFF_CAP_MS = Number(process.env.ADMIN_RATE_LIMIT_BACKOFF_CAP_MS) || 60 * 60 * 1000; // 1 hour
  ...
  return { DEFAULT_MAX, DEFAULT_WINDOW_MS, BACKOFF_BASE_MS, BACKOFF_MULTIPLIER, BACKOFF_CAP_MS, backoffForAttempt };
}

async function isRateLimited(ip: string, redisClient?: any) {
  ...
  const now = Date.now();
  try {
    metrics && typeof metrics.report === 'function' && metrics.report({
...
async function incrementAttempt(ip: string, redisClient?: any) {
  const { DEFAULT_WINDOW_MS, DEFAULT_MAX, BACKOFF_BASE_MS, BACKOFF_MULTIPLIER, BACKOFF_CAP_MS, backoffForAttempt } = getConfig();
  const redis = redisClient || getRedisClient();
  const key = `admin:rl:${ip}`;
  const now = Date.now();
  ...
  local raw = redis.call('GET', key)
    ...
    if blockedUntil > now then
      -- still blocked, return current blockedUntil and count
      return {tostring(count), tostring(blockedUntil)}
    end

    -- increment count and set TTL if first
    count = count + 1
    -- If count exceeds threshold, compute backoff and set blockedUntil
    local blocked = 0
  ...
  const raw = await redis.eval(script, 1, key, DEFAULT_WINDOW_MS, DEFAULT_MAX, now, BACKOFF_BASE_MS, BACKOFF_MULTIPLIER, BACKOFF_CAP_MS);
  // res is array [count, blockedUntil]
...
// CommonJS fallback for test runners that require the module at runtime (ts-node / mocha variations)
try {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (module as any).exports = Object.assign((module as any).exports || {}, { isRateLimited, incrementAttempt });
} catch (e) {
  // ignore in environments where module is not writable
}
// keep CommonJS fallback for test runners
